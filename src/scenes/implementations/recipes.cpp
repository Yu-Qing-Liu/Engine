...
void Recipes::swapChainUpdate() {
	...
	grid->setGridItem = [&](int idx, float x, float y, vec2 cellSize, MVP mvp) {
		if (idx == grid->numItems) {
			...
			recipeNames[idx]->mvp = mvp;
			recipeNames[idx]->text = recipes[idx].name;
			recipeNames[idx]->params.center = vec2(x, y);
			recipeNames[idx]->params.dim = vec2(cellSize.x - 20, cellSize.y - 20);
			recipeNames[idx]->swapChainUpdate();
		}
	};
