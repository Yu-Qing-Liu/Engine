...
void Default::renderPass() {
    ...
    //     mat4(1.0f),
    //     lookAt(vec3(2.0f, 2.0f, 2.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 0.0f, 2.0f)),
    //     perspective(radians(45.0f), Engine::swapChainExtent.width / (float) Engine::swapChainExtent.height, 0.1f, 10.0f)
    // );
    // models->shapes[Models::RECTANGLE]->render();


    // models->textures[Models::IMAGE]->setUniformBuffer(
    //     mat4(1.0f),
    //     lookAt(vec3(2.0f, 2.0f, 2.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 0.0f, 1.0f)),
    //     perspective(radians(45.0f), Engine::swapChainExtent.width / (float) Engine::swapChainExtent.height, 0.1f, 10.0f)
    // );
    models->textures[Models::IMAGE]->setOnFrameUpdate([](Model &logo) {
    ...
    models->textures[Models::IMAGE]->render();
}

void Default::drawFrame() {
    models->textures[Models::IMAGE]->draw();
}
