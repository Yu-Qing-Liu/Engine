...
inline void openDB() {
	...
	// Enforce foreign keys for cascading deletes, etc.
	char *errmsg = nullptr;
	const int rc_fk = sqlite3_exec(conn.get(), "PRAGMA foreign_keys = ON;", nullptr, nullptr, &errmsg);
	if (rc_fk != SQLITE_OK) {
		std::string m = errmsg ? errmsg : "unknown";
		sqlite3_free(errmsg);
		throw std::runtime_error("Failed to enable foreign_keys: " + m);
	}
}
...
inline void printTables() {
	// Only list user tables/views for clarity
	...
	std::cout << "=== Objects in database ===\n";
	while ((rc = sqlite3_step(list_stmt)) == SQLITE_ROW) {
		const char *type = reinterpret_cast<const char *>(sqlite3_column_text(list_stmt, 0));
		const char *name = reinterpret_cast<const char *>(sqlite3_column_text(list_stmt, 1));
		if (!type || !name)
			continue;

		std::cout << "\n-- " << type << ": " << name << " --\n";

		// Skip PRAGMA for SQLite internal tables
		if (std::string(type) == "table" && std::string(name).rfind("sqlite_", 0) != 0) {
			// 1) Fetch column names
			std::vector<std::string> columns;
			{
				std::string pragma_sql = "PRAGMA table_info(" + std::string(name) + ");";
			...
			// 2) Select all rows
			{
				std::string select_sql = "SELECT * FROM " + std::string(name) + ";";
				sqlite3_stmt *select_stmt = nullptr;
				if (sqlite3_prepare_v2(conn.get(), select_sql.c_str(), -1, &select_stmt, nullptr) == SQLITE_OK) {
					const int ncols = sqlite3_column_count(select_stmt);
					...
					std::cerr << "Failed to query rows for table " << name << ": " << sqlite3_errmsg(conn.get()) << "\n";
				}
	...
	if (rc != SQLITE_DONE) {
		std::cerr << "Error iterating sqlite_master: " << sqlite3_errmsg(conn.get()) << "\n";
	}
