    ...
    case APP_CMD_INIT_WINDOW:
        if (app->window) {
            if (instance == VK_NULL_HANDLE) {
                ...
                createSyncObjects();
                currentFrame = 0;
                if (S->scenes) S->scenes->swapChainUpdate();
            }
            S->ready     = true;
            S->animating = true;
        }
...
// <<< ADD: input handler to feed Events::pointerX/Y (used by ray tracing pick)
static int32_t handle_input(android_app* app, AInputEvent* event) {
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
        const int action = AMotionEvent_getAction(event) & AMOTION_EVENT_ACTION_MASK;
        const size_t idx = (AMotionEvent_getAction(event) & AMOTION_EVENT_ACTION_POINTER_INDEX_MASK)
...
void android_main(android_app* app) {
    ...
    app->onInputEvent= handle_input; // <<< ADD: hook input so pointer is updated
